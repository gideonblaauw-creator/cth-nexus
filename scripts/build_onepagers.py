...
def sliders(items: list[dict]) -> str:
    ...
    for i, s in enumerate(items):
        ...
        parts.append(
            ...
            f'<div class="sl-val" id="{sid}-out">{s["prefix"]}{s["min"]}{s["suffix"]}</div>'
            f'<input class="sl-range" type="range" id="{sid}" min="{s["min"]}" max="{s["max"]}" step="{s.get("step",1)}" value="{s["min"]}" '
...
def team_grid(members: list[dict]) -> str:
    cards = []
    for m in members:
        ini = m.get("initials") or "".join(w[0] for w in m["name"].split()[:2]).upper()
        cards.append(
            f'<div class="team-card"><div class="tp-avatar" style="background:linear-gradient(135deg,var(--fo),var(--lg));">{ini}</div>'
            f'<div class="tp-name">{m["name"]}</div>'
            f'<div class="tp-role">{bi(m["role_en"], m["role_es"])}</div>'
            f'<div class="tp-bio">{bi(m.get("bio_en",""), m.get("bio_es",""))}</div></div>'
        )
    ...
    for sec in d["sections"]:
        ...
        if sec["type"] == "split":
            inner = (
                ...
                f'<div class="split-panel"><div class="split-h">{bi(sec["head_en"], sec["head_es"])}</div>'
                f'<div class="body-text">{bi(sec["body_en"], sec["body_es"])}</div></div>'
                f'<div class="split-panel">{stats}{sec.get("extra_html","")}</div></div>'
            )
            ...
            if sec.get("note_en"):
                inner += f'<div style="padding:0 48px 20px;font-size:10px;color:rgba(255,255,255,.45);">{bi(sec["note_en"], sec["note_es"])}</div>'
        elif sec["type"] == "table":
            ...
            inner = team_grid(sec["members"])
            if sec.get("dream_en"):
                inner += (
                    f'<div style="padding:0 48px 28px;font-size:12px;line-height:1.6;color:rgba(255,255,255,.55);">'
                    f'{bi(sec["dream_en"], sec["dream_es"])}</div>'
                )
        elif sec["type"] == "html":
            inner = sec["html"]
...
<div class="sec" style="position:relative;">
  ...
  <div class="cta">
    ...
    <div class="cta-links">
      <a class="cta-link" href="https://nexus.cleantechhub.net/s/{slug}">{bi("Nexus Profile (draft)", "Perfil Nexus (borrador)")}</a>
      <a class="cta-link" href="https://nexus.cleantechhub.net/p/startup-portfolio">{bi("157 Startups", "157 Startups")}</a>
    </div>
    <div class="cta-brand"><div class="tg">Inspira. Actúa. Transforma.</div><div class="lg2">CleantechHUB</div></div>
    <div class="cta-port">{bi("Draft preview — not deployed to /opt/nexus-onepagers", "Vista previa borrador — no desplegado en /opt/nexus-onepagers")}</div>
  </div>
</div>
